...
function IsLupin($c) {
  if ($c.B -gt 145 -and $c.B -gt ($c.R + 30) -and $c.B -gt ($c.G + 10) -and $c.R -lt 175 -and $c.G -lt 210) { return $true }
  if ($c.R -gt 210 -and $c.B -gt 160 -and $c.G -lt 190 -and ($c.R - $c.G) -gt 45) { return $true } # blush
  return $false
}
function IsPillBlue($c) {
  return ($c.B -gt 170 -and $c.R -lt 130 -and $c.G -gt 90 -and $c.G -lt 190 -and ($c.B - $c.R) -gt 70 -and ($c.B - $c.G) -gt 20)
}
...
# --- flagged red castle sprite + bake (same as before) ---
...
function ErasePaintRed($bmp, $castle) {
  $bands = FindBands $bmp
  $teal = $bmp.GetPixel(8, 50)
  foreach ($b in $bands) {
    $ex0 = [Math]::Max(0, $b.x0 - 14); $ex1 = [Math]::Min($bmp.Width - 1, $b.x1 + 14)
    $ey0 = [Math]::Max(0, $b.y0 - 48); $ey1 = [Math]::Min($bmp.Height - 1, $b.y1 + 10)
  ...
  foreach ($b in $bands) {
    ...
    $dy = [Math]::Max(0, $b.y1 - $castle.Height + 8)
    $g.DrawImage($castle, $dx, $dy, $castle.Width, $castle.Height)
  }
  $g.Dispose()
  Write-Output ("red castles baked: {0}" -f $bands.Count)
}

ErasePaintRed $bridge $castle
ErasePaintRed $segment $castle

# --- Erase baked lupin ONLY (tight) ---
function EraseLupinAndPill($bmp) {
  ...
  if ($maxX -lt 0) { Write-Output 'no lupin'; return }
  Write-Output ("lupin x={0}..{1} y={2}..{3}" -f $minX, $maxX, $minY, $maxY)

  # sample castle top color from yellow-green castle below character feet
  $castleSample = $bmp.GetPixel([int](($minX + $maxX) / 2), [Math]::Min($bmp.Height - 1, $maxY + 8))
  $skySample = $bmp.GetPixel(20, 20)

  $ex0 = [Math]::Max(0, $minX - 8); $ex1 = [Math]::Min($bmp.Width - 1, $maxX + 8)
  $ey0 = [Math]::Max(0, $minY - 8); $ey1 = [Math]::Min($bmp.Height - 1, $maxY + 4)
  $pts = New-Object System.Collections.Generic.List[object]
  for ($y = $ey0; $y -le $ey1; $y++) {
    for ($x = $ex0; $x -le $ex1; $x++) {
      $c = $bmp.GetPixel($x, $y)
      $kill = $false
      if (IsLupin $c) { $kill = $true }
      elseif ($c.R -lt 55 -and $c.G -lt 55 -and $c.B -lt 55) {
        foreach ($d in @(@(1,0),@(-1,0),@(0,1),@(0,-1),@(2,0),@(0,2))) {
          $nx = $x + $d[0]; $ny = $y + $d[1]
          if ($nx -ge $ex0 -and $nx -le $ex1 -and $ny -ge $ey0 -and $ny -le $ey1 -and (IsLupin ($bmp.GetPixel($nx, $ny)))) {
            $kill = $true; break
          }
        }
      }
  ...
  foreach ($p in $pts) {
    $x = $p[0]; $y = $p[1]
    # upper part → sky; lower (feet on battlement) → castle sample / neighbor
    if ($y -lt ($minY + ($maxY - $minY) * 0.75)) {
      $bmp.SetPixel($x, $y, $skySample)
    } else {
      $bmp.SetPixel($x, $y, $castleSample)
    }
  }
  Write-Output ("erased lupin px={0}" -f $pts.Count)

  # pill below — only solid pill band (narrow height), not huge region
  $pMinX = 999; $pMaxX = -1; $pMinY = 999; $pMaxY = -1
  for ($y = ($maxY + 5); $y -lt [Math]::Min($bmp.Height, $maxY + 80); $y++) {
    $rowCnt = 0; $rmin = -1; $rmax = -1
    for ($x = 30; $x -lt 160; $x++) {
      if (IsPillBlue ($bmp.GetPixel($x, $y))) {
        $rowCnt++; if ($rmin -lt 0) { $rmin = $x }; $rmax = $x
      }
    }
    if ($rowCnt -gt 20 -and ($rmax - $rmin) -gt 40) {
  ...
  if ($pMaxX -gt 0 -and ($pMaxY - $pMinY) -lt 50) {
    Write-Output ("pill x={0}..{1} y={2}..{3}" -f $pMinX, $pMaxX, $pMinY, $pMaxY)
    for ($y = $pMinY; $y -le $pMaxY; $y++) {
      for ($x = $pMinX; $x -le $pMaxX; $x++) {
        $c = $bmp.GetPixel($x, $y)
        if ((IsPillBlue $c) -or ($c.R -gt 220 -and $c.G -gt 220 -and $c.B -gt 220)) {
          $bmp.SetPixel($x, $y, $skySample)
        }
      }
    }
  } else {
    Write-Output 'pill skip (not found / too tall)'
  }
}

EraseLupinAndPill $bridge
...
$castle.Dispose(); $bridge.Dispose(); $segment.Dispose()
