 ...
 * 여기 한 자리에 다시 그린다.
 *
 * **세로 위치:** OS 상태바(~y 0–54, Dynamic Island 기기 더 큼)와 떨어뜨린다.
 * 예전 y=56은 시계·신호 바로 아래라 붙어 보이고, 큰 상태바에서는 아이콘과 겹칠 수
 * 있다. **y=74**로 내려 여유를 둔다(탭 영역 74–118). 가로는 모서리 쪽(x=6).
 *
 * 에셋에 구워진 `<`를 덮는 `BackButtonMask` 좌표는 **원래 자리 그대로** 둔다
 * (덮개는 그림을 가리는 것이고, 새 화살표만 여기로 옮긴 것이다).
 */
export const BACK_BUTTON_HIT: FigmaNavRect = { x: 6, y: 74, w: 44, h: 44 }

/**
 * 뒤로가기 `<` 색.
 * 예전 `#A0AEB9`는 흰·하늘 배경에서 잘 안 보였다 → 한 단계 진한 슬레이트.
 */
export const BACK_CHEVRON_COLOR = '#5B6B7A'

/** `<` 획 크기 — 히트(44×44) 안. 살짝 키워 가독성 확보 */
export const BACK_CHEVRON = { w: 16, h: 22, strokeWidth: 2.8 } as const
