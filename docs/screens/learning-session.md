...
### 과제 풀이는 단어 4개씩 한 세트로 돈다 (2026-09-14)

`buildAssignmentSections`는 **단어를 4개씩 묶어 그 세트로 켜진 유형을 다 돌고** 다음
세트로 넘어간다. 유형별로 몰아서 내면 20단어 과제에서 짝맞추기만 5판을 내리 풀게 된다.

```
[1~4단어] A → B → C → D
[5~8단어] A → B → C → D …   (본문·문법은 단어가 다 끝난 뒤)
```
...
- 마지막 세트가 4개가 안 되면 **A·B는 `fillPool`에서 다른 단어를 끌어와 4짝을 채우고**
  ...
  낸다. 채움 단어까지 다시 물으면 같은 단어를 두 번 푸는 꼴이 된다.
