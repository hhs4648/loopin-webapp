...
4. `praise-calendar` — 칭찬 캘린더 (맵 버튼 → `PraiseCalendarScreen`)
   ...
   - 날짜 키: 과제 `lessonDate` (칸에 붙는 날)
   - 이모티콘 **발동**: 해당 과제를 **완료**했거나 **마감일**(`deadlineDate`+`deadlineTime`, 없으면 수업일)이 **지났을 때만**. 그 전에는 칸에 얼굴 없음
   - 얼굴 종류: 완료·점수 ≥통과기준 → 통과 / 완료·미만 → 아쉬움 / 미제출·마감 후 → 미제출
   - 같은 날 복수 과제: 하나라도(마감 후) 미제출이면 미제출, 모두 완료면 평균 점수. 마감 전 미풀이는 집계에서 제외. 오답 재출제(개인 과제)는 제외
   - 하단 내비: 시안 베이크는 흰 덮개로 가리고 `MainHomeBottomNav` 사용(연속 학습 캘린더와 동일). 복습·전체는 맵으로 돌아간 뒤 창을 연다
...
| 고정 하늘 | **제거(2026-09-14).** 하늘 그라데이션(`MAIN_HOME_SKY_GRADIENT`)도 풀맵과 **같이 스크롤**. 연속 학습 배지도 하늘과 함께 움직임 |
